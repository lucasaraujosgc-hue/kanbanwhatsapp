...
const dbPath = require('fs').existsSync(path.join(DATA_DIR, 'kanban.db')) ? path.join(DATA_DIR, 'kanban.db') : path.join(__dirname, 'data', 'kanban.db');
...
db.all("SELECT * FROM chats WHERE id LIKE '%lid%'", (err, rows) => {
  if (err) return console.error(err);
